import {
  useCallback, useEffect, useMemo, useReducer, useRef, useState, type ChangeEvent,
} from 'react';
import { motion } from 'framer-motion';
import { en } from '@/i18n/locales/en.ts';
import { requestApi } from '@/admin/api/requests.ts';
import type {
  BudgetFlexibility,
  BudgetRange,
...
import { contactMessageApi } from '@/admin/api/contactMessages.ts';
import { normalizeApiError } from '@/admin/api/client.ts';
import { useSettingsOptions } from '@/admin/hooks/useSettingsOptions.ts';
import { premiumEase, premiumMotion } from '@/lib/motion.ts';
import { usePublicSettings } from '@/settings/usePublicSettings.ts';
...
export const Contact = () => {
  ...
  const detailedSteps = useMemo(
    () => [
      detailedCopy.contactInfo,
      detailedCopy.projectEssentials,
      ...(!detailedRequest.enterpriseInquiry ? [detailedCopy.extraContext] : []),
    ],
  ...
  const handleNextStep = useCallback(() => {
    ...
    if (hasErrorsInStep(validationErrors, uiState.currentStep)) {
      ...
      dispatch({ type: 'SUBMIT_ERROR', errorText: 'Please complete the required fields in this step.' });
      return;
    }
      ...
      try {
        if (!useDetailedRequest) {
          ...
          if (Object.keys(validationErrors).length > 0) {
            setFieldErrors(validationErrors);
            dispatch({ type: 'SUBMIT_ERROR', errorText: 'Please review the highlighted fields and try again.' });
  ...
  const handleDetailedFieldChange = useCallback(
