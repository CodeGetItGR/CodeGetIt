...
import type { DetailedRequestFormState, SubmitState } from './useUIReducer';
import type { ContactFormData, ContactOptions } from './contact.types';
import { DetailedRequestWizard } from './DetailedRequestWizard';
...
const Field = ({ label, placeholder, value, onChange, error, type = 'text', required }: FieldProps) => (
  <div>
    <label className="mb-3 block text-sm font-medium uppercase tracking-wider text-gray-500">{label}</label>
    {type === 'textarea' ? (
      <textarea
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {useDetailedRequest && currentStep < stepTitles.length - 1 ? (
            <button
              type="button"
              onClick={onNextStep}
              className="rounded-full border border-gray-900 bg-blue-900 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-black cursor-pointer"
            >
